import nuiTabsItem from './nuiTabsItem'
import nuiTabs from './nuiTabs'
...
const DefaultTemplate = (args, { argTypes }) => ({
  components: { nuiTabs, nuiTabsItem },
  props: Object.keys(argTypes),
  template: `
    <nuiTabs v-bind="$props">
        <nuiTabsItem v-for="(item, $index) in tabs" :key="$index" :label="item.title">{{ item.content }}</nuiTabsItem>
    </nuiTabs>
  `
})
